...
public class StateGenerating extends DefaultOrder implements State {
    ...
    public void switchFromGeneratingToPlaying(Maze maze) {
        ...
        //Hand over the newly generated maze and controller to the robot and driver in the control object if they exist
        if (control.getRobot() != null && control.getDriver() != null) {
        	//Set up robot with control
        	Robot robot = control.getRobot();
        	robot.setController(control);
        	
   
        	//Set up sensors with robot
        	DistanceSensor sensorF = null;
        	DistanceSensor sensorL = null;
        	DistanceSensor sensorR = null;
        	DistanceSensor sensorB = null;
        	
        	//Pull corresponding values from configuration parameter to determine which sensors are to be placed
        	char configF = control.sensorConfig.charAt(0);
        	char configL = control.sensorConfig.charAt(1);
        	char configR = control.sensorConfig.charAt(2);
        	char configB = control.sensorConfig.charAt(3);
        	
        	//Go through each configuration digit and set corresponding sensor
        	if (configF == '0') {
        	...
        	//Activate fail and repair process for unreliable sensors
        	if (configF == '0') {
        	...
        	//Reset robot's internal state
        	//Addresses bug where robot would not fully reset after a round ended
        	robot.setBatteryLevel(3500);
        	robot.resetOdometer();
        	if (robot instanceof ReliableRobot) {
        		((ReliableRobot) robot).resetHasStopped();
        	}
        	if (robot instanceof UnreliableRobot) {
        		((UnreliableRobot) robot).resetHasStopped();
        	}
        	
        	//Set up driver with maze & robot
        	RobotDriver driver = control.getDriver();
        	driver.setMaze(maze);
        	driver.setRobot(robot);
        	//Give it back to control
