...
public class StatePlaying implements State {
     ...
     */
    public void switchFromPlayingToWinning(int pathLength, int winState) {
        ...
        //If we are playing with a human player, hand over standard win screen
        if (winState == 0) {
        	currentState.start(control, panel);
        }
        //Else call for one of automated end screens
        else {
        	currentState.start(control, panel, winState);
        }
    }
    ...
    public boolean handleUserInput(UserInput userInput, int value) {
        ...
        case UP: // move forward
            ...
            if (isOutside(px,py)) {
                ...
                switchFromPlayingToWinning(0, 0);
            }
            break;
        case LEFT: // turn left
